// ------------------------------------------------------------------------------
//  Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.  See License in the project root for license information.
// ------------------------------------------------------------------------------

// **NOTE** This file was generated by a tool and any changes will be overwritten.

// Template Source: Templates\CSharp\Requests\IEntityRequest.cs.tt

namespace Microsoft.Graph
{
    using System;
    using System.IO;
    using System.Net.Http;
    using System.Threading;
    using System.Linq.Expressions;

    /// <summary>
    /// The interface IUpdateMetadataOperationRequest.
    /// </summary>
    public partial interface IUpdateMetadataOperationRequest : IBaseRequest
    {
        /// <summary>
        /// Creates the specified UpdateMetadataOperation using PUT.
        /// </summary>
        /// <param name="updateMetadataOperationToCreate">The UpdateMetadataOperation to create.</param>
        /// <returns>The created UpdateMetadataOperation.</returns>
        System.Threading.Tasks.Task<UpdateMetadataOperation> CreateAsync(UpdateMetadataOperation updateMetadataOperationToCreate);        /// <summary>
        /// Creates the specified UpdateMetadataOperation using PUT.
        /// </summary>
        /// <param name="updateMetadataOperationToCreate">The UpdateMetadataOperation to create.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> for the request.</param>
        /// <returns>The created UpdateMetadataOperation.</returns>
        System.Threading.Tasks.Task<UpdateMetadataOperation> CreateAsync(UpdateMetadataOperation updateMetadataOperationToCreate, CancellationToken cancellationToken);

        /// <summary>
        /// Deletes the specified UpdateMetadataOperation.
        /// </summary>
        /// <returns>The task to await.</returns>
        System.Threading.Tasks.Task DeleteAsync();

        /// <summary>
        /// Deletes the specified UpdateMetadataOperation.
        /// </summary>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> for the request.</param>
        /// <returns>The task to await.</returns>
        System.Threading.Tasks.Task DeleteAsync(CancellationToken cancellationToken);

        /// <summary>
        /// Gets the specified UpdateMetadataOperation.
        /// </summary>
        /// <returns>The UpdateMetadataOperation.</returns>
        System.Threading.Tasks.Task<UpdateMetadataOperation> GetAsync();

        /// <summary>
        /// Gets the specified UpdateMetadataOperation.
        /// </summary>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> for the request.</param>
        /// <returns>The UpdateMetadataOperation.</returns>
        System.Threading.Tasks.Task<UpdateMetadataOperation> GetAsync(CancellationToken cancellationToken);

        /// <summary>
        /// Updates the specified UpdateMetadataOperation using PATCH.
        /// </summary>
        /// <param name="updateMetadataOperationToUpdate">The UpdateMetadataOperation to update.</param>
        /// <returns>The updated UpdateMetadataOperation.</returns>
        System.Threading.Tasks.Task<UpdateMetadataOperation> UpdateAsync(UpdateMetadataOperation updateMetadataOperationToUpdate);

        /// <summary>
        /// Updates the specified UpdateMetadataOperation using PATCH.
        /// </summary>
        /// <param name="updateMetadataOperationToUpdate">The UpdateMetadataOperation to update.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/> for the request.</param>
        /// <returns>The updated UpdateMetadataOperation.</returns>
        System.Threading.Tasks.Task<UpdateMetadataOperation> UpdateAsync(UpdateMetadataOperation updateMetadataOperationToUpdate, CancellationToken cancellationToken);

        /// <summary>
        /// Adds the specified expand value to the request.
        /// </summary>
        /// <param name="value">The expand value.</param>
        /// <returns>The request object to send.</returns>
        IUpdateMetadataOperationRequest Expand(string value);

        /// <summary>
        /// Adds the specified expand value to the request.
        /// </summary>
        /// <param name="expandExpression">The expression from which to calculate the expand value.</param>
        /// <returns>The request object to send.</returns>
        IUpdateMetadataOperationRequest Expand(Expression<Func<UpdateMetadataOperation, object>> expandExpression);

        /// <summary>
        /// Adds the specified select value to the request.
        /// </summary>
        /// <param name="value">The select value.</param>
        /// <returns>The request object to send.</returns>
        IUpdateMetadataOperationRequest Select(string value);

        /// <summary>
        /// Adds the specified select value to the request.
        /// </summary>
        /// <param name="selectExpression">The expression from which to calculate the select value.</param>
        /// <returns>The request object to send.</returns>
        IUpdateMetadataOperationRequest Select(Expression<Func<UpdateMetadataOperation, object>> selectExpression);

    }
}

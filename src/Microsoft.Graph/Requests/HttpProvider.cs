            ...
            set
            {
                ...
                catch (InvalidOperationException exception)
                {
                    throw new ServiceException(
                        new Error
                        {
                            Code = GraphErrorCode.NotAllowed.ToString(),
            ...
            if (!response.IsSuccessStatusCode && !this.IsRedirect(response.StatusCode))
            {
                using (response)
                {
                    ...
                    if (errorResponse == null || errorResponse.Error == null)
                    {
                        ...
                        {
                            error = new Error { Code = GraphErrorCode.ItemNotFound.ToString() };
                        }
                        else
                        {
                            error = new Error
                ...
                if (this.IsRedirect(response.StatusCode))
                {
                    if (++redirectCount > HttpProvider.maxRedirects)
                    {
                        throw new ServiceException(
                            new Error
                            {
                                Code = GraphErrorCode.TooManyRedirects.ToString(),
            ...
            CancellationToken cancellationToken)
        {
            ...
            catch (TaskCanceledException exception)
            {
                throw new ServiceException(
                        new Error
                        {
                            Code = GraphErrorCode.Timeout.ToString(),
